...
const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        'grid w-full auto-rows-[10rem] md:auto-rows-[14rem] grid-cols-2 md:grid-cols-3 gap-4 md:gap-6',
        className
      )}
    >
      {children}
...
const BentoCard = ({
  ...
  >
    <div>{background}</div>
    <div className='pointer-events-none z-10 flex transform-gpu flex-col gap-2 md:gap-3 p-4 md:p-8 transition-all duration-300 group-hover:-translate-y-2'>
      <h3 className='text-base md:text-xl font-serif font-semibold text-white leading-tight drop-shadow-lg'>
        {name}
      </h3>
      <p className='hidden md:block text-white/90 text-sm leading-relaxed max-w-lg drop-shadow-md'>
        {description}
      </p>
    </div>
